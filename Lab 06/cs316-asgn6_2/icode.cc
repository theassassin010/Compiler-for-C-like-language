...
{
	//TODO
	CHECK_INVARIANT((symbol_entry != NULL), "Symbol Entry cannot be Null")
...
Control_Flow_IC_Stmt::Control_Flow_IC_Stmt(Tgt_Op inst_op, Ics_Opd * o1, Ics_Opd * o2, string label){
	CHECK_INVARIANT((machine_desc_object.spim_instruction_table[inst_op] != NULL),
			"Instruction description in spim table cannot be null");
...
Control_Flow_IC_Stmt::Control_Flow_IC_Stmt(Tgt_Op op, string label)
{
	CHECK_INVARIANT((machine_desc_object.spim_instruction_table[op] != NULL),
			"Instruction description in spim table cannot be null");
...
void Code_For_Ast::append_ics(Icode_Stmt & ic_stmt)
{
	...
	Ics_Opd * res = ic_stmt.get_result();
	result_register = res -> get_reg();
}
